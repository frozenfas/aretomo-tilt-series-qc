...
def _read_ts_metadata(aretomo_dir, prefix, dose_override=None):
    ...
    # Tilt angles: the .aln TILT column is always the nominal (uncorrected)
    # stage tilt -- AreTomo3 never bakes AlphaOffset into it, whether it came
    # from -TiltCor's automatic estimate or was set via aln-edit.  Apply it
    # here to get the specimen-referenced tilt (same convention as
    # pytom_match.py / relion5_convert.py).
...
def _build_extract_script(scores_em, angles_em, angles_list, tomo_id,
    ...
    return (
        ...
        'zxz = np.stack([motl.df["phi"].values, motl.df["theta"].values, motl.df["psi"].values], axis=1); '
        'zyz = R.from_euler("ZXZ", zxz, degrees=True).as_euler("ZYZ", degrees=True); '
        'df = pd.DataFrame({'
...
def _run_extraction(ts_out, tomo_id, angpix, angles_list, python_bin, args,
                    dry_run=False, threshold_override=None):
    ...
    threshold   = threshold_override if threshold_override is not None else args.scores_threshold
...
def add_parser(subparsers):
    ...
    ext.add_argument('--scores-threshold', type=float, default=0.08,
                     help='Score threshold for particle extraction (global, used when '
                          '--scores-threshold-csv is not provided)')
    ext.add_argument('--scores-threshold-csv', default=None, metavar='CSV',
                     help='CSV file with per-TS thresholds (columns: ts_name,threshold). '
                          'Generated by the "Download thresholds CSV" button in the '
                          'interactive QC report. Overrides --scores-threshold for '
                          'tomograms listed in the file.')
...
def _run_extract_only(args, out_dir, sep):  # noqa: C901
    ...
    # Load per-TS thresholds from CSV if provided
    thr_csv = getattr(args, 'scores_threshold_csv', None)
    per_ts_threshold = {}
    if thr_csv:
        per_ts_threshold = _load_threshold_csv(thr_csv)
        print(f'Loaded per-TS thresholds from {thr_csv} ({len(per_ts_threshold)} entries)')
    ...
    for ts_name, ts_dir, param_file in jobs:
        ...
        ok_ts = _run_extraction(
            ...
            threshold_override=per_ts_threshold.get(ts_name),
        )
